...
export async function createMercadoPagoIntent(data: CheckoutPayload, terminalId: string) {
  try {
    const parsed = checkoutSchema.parse(data)
    const externalRef = `mp_${crypto.randomUUID()}`
    const { transactionId: txId, receiptToken } = await processUnifiedTransaction(parsed, 'mercadopago', 'pending', externalRef)

    // Simulation for demo
    setTimeout(async () => {
        const supabase = await createClient()
        // In reality, this happens via webhook which also triggers loyalty 'earn' entry if needed
        await supabase.from('transactions').update({ status: 'completed' }).eq('id', txId)
    }, 5000)

    return { success: true, transactionId: txId, receiptToken, reference: externalRef }
  } catch (error: unknown) {
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
  }
}

export async function createCodiIntent(data: CheckoutPayload) {
  try {
    const parsed = checkoutSchema.parse(data)
    ...
    return { success: true, transactionId: txId, receiptToken, qrPayload: qrPayloadString }
  } catch (error: unknown) {
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
  }
}
